...
wall_area_north = (height - height_ground_wall) * (
        width * math.cos(inclination * math.pi / 180) + length * math.sin(inclination * math.pi / 180))
wall_area_west = (height - height_ground_wall) * (
        width * math.sin(inclination * math.pi / 180) + length * math.cos(inclination * math.pi / 180))
wall_area_east = wall_area_west
wall_area_south = wall_area_north
wall_area_roof = floor_area
ground_wall_area_north = height_ground_wall * (
        width * math.cos(inclination * math.pi / 180) + length * math.sin(inclination * math.pi / 180))
ground_wall_area_west = height_ground_wall * (
        width * math.sin(inclination * math.pi / 180) + length * math.cos(inclination * math.pi / 180))
...
# 熱源のパラメータ
q_ref_rated_cool = 703 * 2  # 熱源の定格冷却能力 703[kW/台]で2ユニット想定，これが熱源のパラメータになる
q_ref_rated_heat = 588 * 2
e_ref_cool = q_ref_rated_cool  # 熱源の消費エネルギー，冷却は定格能力と同じと仮定
e_ref_heat = q_ref_rated_heat * 1.5  # 散逸があるので定格能力の1.5倍
e_ref_sub_cool = e_ref_cool * 0.01  # 熱源補機の消費エネルギー（これはErefの1%でいいんでない？）
e_ref_sub_heat = e_ref_heat * 0.01
primary_pump_power_consumption_total = 7.5 * 2  # 熱源一次ポンプの消費電力トータル（ここも2台分）
cooling_tower_fan_power_consumption_total = 7.5 * 2  # 冷却塔ファンの消費電力トータル（ここも2台分）
cooling_tower_pump_power_consumption_total = 15 * 2  # 冷却塔ポンプの消費電力トータル（ここも2台分），冷却塔ファンの2倍の電力を仮定するでも
cooling_tower_capacity_total = 1233 * 2
# ２次ポンプのパラメータ
temperature_difference = 5.0
rated_water_flow_rate_total = 89.0
rated_power_consumption_total = 7.5
# 代表的空調機のパラメータ
air_heat_exchange_rate_cooling = 52  # 定格冷却能力 [kW/台]，今は1台を想定
air_heat_exchange_rate_heating = 29  # 定格冷却能力 [kW/台]，今は1台を想定
...
unit_spec_cool = {
    ...
    'heat_source': [
        {'heat_source_type': '吸収式冷凍機(一重二重併用形、都市ガス)',
         'number': 1.0,
         'supply_water_temp_summer': 7.0,
         'supply_water_temp_middle': 7.0,
         'supply_water_temp_winter': 7.0,
         'heat_source_rated_capacity': q_ref_rated_cool,
         'heat_source_rated_power_consumption': 0,
         'heat_source_rated_fuel_consumption': e_ref_cool,
         'heat_source_sub_rated_power_consumption': e_ref_sub_cool,
         'primary_pump_power_consumption': primary_pump_power_consumption_total,
         'primary_pump_control_type': '無',
         'cooling_tower_capacity': cooling_tower_capacity_total,
         'cooling_tower_fan_power_consumption': cooling_tower_fan_power_consumption_total,
         'cooling_tower_pump_power_consumption': cooling_tower_pump_power_consumption_total,
...
unit_spec_heat = {
    ...
    'heat_source': [
        {'heat_source_type': '吸収式冷凍機(一重二重併用形、都市ガス)',
         ...
         'heat_source_rated_capacity': q_ref_rated_heat,
         'heat_source_rated_power_consumption': 0,
         'heat_source_rated_fuel_consumption': e_ref_heat,
         'heat_source_sub_rated_power_consumption': e_ref_sub_heat,
         'primary_pump_power_consumption': primary_pump_power_consumption_total,
         'primary_pump_control_type': '無',
...
unit_spec_secondary = {
    ...
    "secondary_pump": [
        {
            "number": 1.0,
            "rated_water_flow_rate": rated_water_flow_rate_total,
            "rated_power_consumption": rated_power_consumption_total,
            "control_type": "定流量制御",
