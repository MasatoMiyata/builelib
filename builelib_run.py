...
def builelib_run(exec_calculation, inputfile_name):
    ...
    if exec_calculation:
        ...
        try:
            if input_data[
                ...
                resultdata_AC = airconditioning_webpro.calc_energy(
                ...
                # 設計一次エネ・基準一次エネに追加
                energy_consumption_design += resultdata_AC[
                    "設計一次エネルギー消費量[MJ/年]"
                ]
                energy_consumption_standard += resultdata_AC[
                    "基準一次エネルギー消費量[MJ/年]"
                ]
                ...
                resultdata_AC = {"message": "空気調和設備はありません。"}
        ...
        resultdata_AC = {"error": "空気調和設備の計算は実行されませんでした。"}
    ...
    if exec_calculation:
        ...
        try:
            if input_data["ventilation_room"]:  # ventilation_room が 空 でなければ
                ...
                # 設計一次エネ・基準一次エネに追加
                energy_consumption_design += resultdata_V[
                    "設計一次エネルギー消費量[MJ/年]"
                ]
                energy_consumption_standard += resultdata_V[
                    "基準一次エネルギー消費量[MJ/年]"
                ]
                calc_reuslt["設計一次エネルギー消費量（換気）[MJ]"] = resultdata_V[
                    "設計一次エネルギー消費量[MJ/年]"
                ]
                calc_reuslt["基準一次エネルギー消費量（換気）[MJ]"] = resultdata_V[
                    "基準一次エネルギー消費量[MJ/年]"
                ]
                calc_reuslt["BEI_V"] = math.ceil(resultdata_V["BEI/V"] * 100) / 100

            else:
                resultdata_V = {"message": "機械換気設備はありません。"}
        ...
        resultdata_V = {"error": "機械換気設備の計算は実行されませんでした。"}
    ...
    if exec_calculation:
        ...
        try:
            if input_data["lighting_systems"]:  # lighting_systems が 空 でなければ
                ...
                calc_reuslt["設計一次エネルギー消費量（照明）[MJ]"] = resultdata_L[
                    "E_lighting"
                ]
                calc_reuslt["基準一次エネルギー消費量（照明）[MJ]"] = resultdata_L[
                    "Es_lighting"
                ]
                calc_reuslt["BEI_L"] = math.ceil(resultdata_L["BEI_L"] * 100) / 100

            else:
                resultdata_L = {"message": "照明設備はありません。"}

        except:
            resultdata_L = {"error": "照明設備の計算時に予期せぬエラーが発生しました。"}

    else:
        resultdata_L = {"error": "照明設備の計算は実行されませんでした。"}
    ...
    if exec_calculation:
        ...
        try:
            if input_data["hot_water_room"]:  # hot_water_room が 空 でなければ
                ...
                calc_reuslt["設計一次エネルギー消費量（給湯）[MJ]"] = result_data_HW[
                    "設計一次エネルギー消費量[MJ/年]"
                ]
                calc_reuslt["基準一次エネルギー消費量（給湯）[MJ]"] = result_data_HW[
                    "基準一次エネルギー消費量[MJ/年]"
                ]
    ...
    if exec_calculation:
        ...
        try:
            if input_data["elevators"]:  # elevators が 空 でなければ
                ...
                calc_reuslt["設計一次エネルギー消費量（昇降機）[MJ]"] = resultdata_EV[
                    "E_elevator"
                ]
                calc_reuslt["基準一次エネルギー消費量（昇降機）[MJ]"] = resultdata_EV[
                    "Es_elevator"
                ]
                calc_reuslt["BEI_EV"] = math.ceil(resultdata_EV["BEI_EV"] * 100) / 100

            else:
                resultdata_EV = {"message": "昇降機はありません。"}

        except:
            resultdata_EV = {"error": "昇降機の計算時に予期せぬエラーが発生しました。"}

    else:
        resultdata_EV = {"error": "昇降機の計算は実行されませんでした。"}
    ...
    if exec_calculation:
        ...
        try:
            if input_data[
                ...
                calc_reuslt["創エネルギー量（太陽光）[MJ]"] = resultdata_PV[
                    "E_photovoltaic"
                ]

            else:
                resultdata_PV = {"message": "太陽光発電設備はありません。"}
        except:
            resultdata_PV = {
                "error": "太陽光発電設備の計算時に予期せぬエラーが発生しました。"
            }
    else:
        resultdata_PV = {"error": "太陽光発電設備の計算は実行されませんでした。"}
    ...
    if exec_calculation:
        ...
        try:
            if input_data["rooms"]:  # rooms が 空 でなければ
                ...
                resultdata_OT = {"message": "その他一次エネルギー消費量は0です。"}
        except:
            resultdata_OT = {
    ...
    # 出力
    with open(
        inputfile_name_split[0] + "_result_Other.json", "w", encoding="utf-8"
    ) as fw:
    ...
    if exec_calculation:
        ...
        try:
            if input_data[
                ...
                resultdata_CGS = cogeneration.calc_energy(
                ...
                calc_reuslt["創エネルギー量（コジェネ）[MJ]"] = (
                    resultdata_CGS["年間一次エネルギー削減量"] * 1000
                )

            else:
                resultdata_CGS = {"message": "コージェネレーション設備はありません。"}
        except:
    ...
    # 出力
    with open(
        inputfile_name_split[0] + "_result_CGS.json", "w", encoding="utf-8"
    ) as fw:
    ...
    if energy_consumption_standard != 0:
        ...
        calc_reuslt["設計一次エネルギー消費量（その他除き）[MJ]"] = (
            energy_consumption_design
        )
        calc_reuslt["基準一次エネルギー消費量（その他除き）[MJ]"] = (
        ...
        calc_reuslt["設計一次エネルギー消費量（再エネ、その他除き）[MJ]"] = (
            energy_consumption_design + calc_reuslt["創エネルギー量（太陽光）[MJ]"]
        )

        calc_reuslt["BEI（再エネ除き）"] = (
            calc_reuslt["設計一次エネルギー消費量（再エネ、その他除き）[MJ]"]
            / calc_reuslt["基準一次エネルギー消費量（その他除き）[MJ]"]
        )
        calc_reuslt["BEI（再エネ除き）"] = (
            math.ceil(calc_reuslt["BEI（再エネ除き）"] * 100) / 100
        )

        # 設計一次エネ・基準一次エネにその他を追加
        if "E_other" in resultdata_OT:
            calc_reuslt["設計一次エネルギー消費量[MJ]"] = (
                energy_consumption_design + resultdata_OT["E_other"]
            )
            calc_reuslt["基準一次エネルギー消費量[MJ]"] = (
                energy_consumption_standard + resultdata_OT["E_other"]
            )
    ...
    with open(
        inputfile_name_split[0] + "_validation.json", "w", encoding="utf-8"
    ) as fw:
    ...
    with zipfile.ZipFile(
        inputfile_name_split[0] + ".zip", "w", compression=zipfile.ZIP_DEFLATED
    ) as new_zip:
        new_zip.write(
            inputfile_name_split[0] + "_input.json", arcname="builelib_input.json"
        )
        new_zip.write(
            inputfile_name_split[0] + "_validation.json",
            arcname="builelib_validation.json",
        )
        new_zip.write(
            inputfile_name_split[0] + "_result.json", arcname="builelib_result.json"
        )
        new_zip.write(
            inputfile_name_split[0] + "_result_AC.json",
            arcname="builelib_result_AC.json",
        )
        new_zip.write(
            inputfile_name_split[0] + "_result_V.json", arcname="builelib_result_V.json"
        )
        new_zip.write(
            inputfile_name_split[0] + "_result_L.json", arcname="builelib_result_L.json"
        )
        new_zip.write(
            inputfile_name_split[0] + "_result_HW.json",
            arcname="builelib_result_HW.json",
        )
        new_zip.write(
            inputfile_name_split[0] + "_result_EV.json",
            arcname="builelib_result_EV.json",
        )
        new_zip.write(
            inputfile_name_split[0] + "_result_PV.json",
            arcname="builelib_result_PV.json",
        )
        new_zip.write(
            inputfile_name_split[0] + "_result_CGS.json",
            arcname="builelib_result_CGS.json",
        )
        new_zip.write(
            inputfile_name_split[0] + "_result_Other.json",
