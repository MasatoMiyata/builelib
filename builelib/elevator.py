...
def calc_energy(inputdata, DEBUG=False):
    ...
    for room_name in inputdata["Elevators"]:
        for unit_id, unit_configure in enumerate(inputdata["Elevators"][room_name]["Elevator"]):
            ...
            elif unit_configure["ControlType"] == "VVVF(電力回生なし)" or unit_configure[
                "ControlType"] == "VVVF（電力回生なし）":
                inputdata["Elevators"][room_name]["Elevator"][unit_id]["ControlTypeCoefficient"] = 1 / 40

            elif unit_configure["ControlType"] == "VVVF(電力回生あり)" or unit_configure[
                "ControlType"] == "VVVF（電力回生あり）":
                inputdata["Elevators"][room_name]["Elevator"][unit_id]["ControlTypeCoefficient"] = 1 / 45

            elif unit_configure["ControlType"] == "VVVF(電力回生なし、ギアレス)" or unit_configure[
                "ControlType"] == "VVVF（電力回生なし、ギアレス）":
                inputdata["Elevators"][room_name]["Elevator"][unit_id]["ControlTypeCoefficient"] = 1 / 45

            elif unit_configure["ControlType"] == "VVVF(電力回生あり、ギアレス)" or unit_configure[
                "ControlType"] == "VVVF（電力回生あり、ギアレス）":
    ...
    for room_name in inputdata["Elevators"]:
        ...
        if buildingType == "共同住宅":
            ...
            inputdata["Elevators"][room_name]["operation_schedule_hourly"] = bc.get_dailyOpeSchedule_lighting(
                buildingType, roomType, input_calendar)
            inputdata["Elevators"][room_name]["operation_time"] = np.sum(
                np.sum(inputdata["Elevators"][room_name]["operation_schedule_hourly"]))
        ...
        if "room_schedule" in inputdata["SpecialInputData"]:
            ...
            if room_name in inputdata["SpecialInputData"]["room_schedule"]:
                ...
                if "照明発熱密度比率" in inputdata["SpecialInputData"]["room_schedule"][room_name]["schedule"]:
                    inputdata["Elevators"][room_name]["operation_schedule_hourly"] = np.array(
    ...
    for room_name in inputdata["Elevators"]:
        for unit_id, unit_configure in enumerate(inputdata["Elevators"][room_name]["Elevator"]):
            ...
            if DEBUG:
                ...
                print(
                    f'　- エネルギー消費量 kWh/年 {inputdata["Elevators"][room_name]["Elevator"][unit_id]["energy_consumption"]}')
