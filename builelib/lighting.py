...
def calc_energy(inputdata, DEBUG=False):
    ...
    for room_zone_name in inputdata["LightingSystems"]:
        ...
        elif inputdata["LightingSystems"][room_zone_name]["roomWidth"] != None and \
                inputdata["LightingSystems"][room_zone_name]["roomDepth"] != None and \
                inputdata["LightingSystems"][room_zone_name]["unitHeight"] != None:
            if inputdata["LightingSystems"][room_zone_name]["roomWidth"] > 0 and \
                    inputdata["LightingSystems"][room_zone_name]["roomDepth"] > 0 and \
                    inputdata["LightingSystems"][room_zone_name]["unitHeight"] > 0:
                roomIndex = (inputdata["LightingSystems"][room_zone_name]["roomWidth"] *
                             inputdata["LightingSystems"][room_zone_name]["roomDepth"]) / ((inputdata[
                                                                                                "LightingSystems"][
                                                                                                room_zone_name][
                                                                                                "roomWidth"] +
                                                                                            inputdata[
        ...
        for unit_name in inputdata["LightingSystems"][room_zone_name]["lightingUnit"]:
            ...
            if inputdata["LightingSystems"][room_zone_name]["lightingUnit"][unit_name]["OccupantSensingCTRL"] in \
                    lightingCtrl["OccupantSensingCTRL"]:
                # データベースから検索して効果係数を決定
                ctrl_occupant_sensing = lightingCtrl["OccupantSensingCTRL"][
                    inputdata["LightingSystems"][room_zone_name]["lightingUnit"][unit_name]["OccupantSensingCTRL"]]
            else:
                # 直接入力された効果係数を使用
            ...
            if inputdata["LightingSystems"][room_zone_name]["lightingUnit"][unit_name]["IlluminanceSensingCTRL"] in \
                    lightingCtrl["IlluminanceSensingCTRL"]:
                # データベースから検索して効果係数を決定
                ctrl_illuminance_sensing = lightingCtrl["IlluminanceSensingCTRL"][
                    inputdata["LightingSystems"][room_zone_name]["lightingUnit"][unit_name]["IlluminanceSensingCTRL"]]
            else:
                # 直接入力された効果係数を使用
            ...
            if inputdata["LightingSystems"][room_zone_name]["lightingUnit"][unit_name]["TimeScheduleCTRL"] in \
                    lightingCtrl["TimeScheduleCTRL"]:
                # データベースから検索して効果係数を決定
                ctrl_time_schedule = lightingCtrl["TimeScheduleCTRL"][
                    inputdata["LightingSystems"][room_zone_name]["lightingUnit"][unit_name]["TimeScheduleCTRL"]]
            else:
                # 直接入力された効果係数を使用
            ...
            if inputdata["LightingSystems"][room_zone_name]["lightingUnit"][unit_name][
                "InitialIlluminationCorrectionCTRL"] in lightingCtrl["InitialIlluminationCorrectionCTRL"]:
                # データベースから検索して効果係数を決定
                initial_illumination_correction = lightingCtrl["InitialIlluminationCorrectionCTRL"][
                    inputdata["LightingSystems"][room_zone_name]["lightingUnit"][unit_name][
