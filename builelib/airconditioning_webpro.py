...
def calc_energy(inputdata, debug=False):
    ...
    (solor_radiation["直達"]["南西"], solor_radiation["直達_入射角特性込"]["南西"], _,
     _) = climate.solarRadiationByAzimuth(45, 90, phi, longi, IodALL, IosALL, InnALL)
    (solor_radiation["直達"]["西"], solor_radiation["直達_入射角特性込"]["西"], _, _) = climate.solarRadiationByAzimuth(
        90, 90, phi, longi, IodALL, IosALL, InnALL)
    (solor_radiation["直達"]["北西"], solor_radiation["直達_入射角特性込"]["北西"], _,
     _) = climate.solarRadiationByAzimuth(135, 90, phi, longi, IodALL, IosALL, InnALL)
    (solor_radiation["直達"]["北"], solor_radiation["直達_入射角特性込"]["北"], _, _) = climate.solarRadiationByAzimuth(
        180, 90, phi, longi, IodALL, IosALL, InnALL)
    (solor_radiation["直達"]["北東"], solor_radiation["直達_入射角特性込"]["北東"], _,
     _) = climate.solarRadiationByAzimuth(225, 90, phi, longi, IodALL, IosALL, InnALL)
    (solor_radiation["直達"]["東"], solor_radiation["直達_入射角特性込"]["東"], _, _) = climate.solarRadiationByAzimuth(
        270, 90, phi, longi, IodALL, IosALL, InnALL)
    (solor_radiation["直達"]["南東"], solor_radiation["直達_入射角特性込"]["南東"], _,
     _) = climate.solarRadiationByAzimuth(315, 90, phi, longi, IodALL, IosALL, InnALL)
            ...
            if inputdata["WallConfigure"][wall_configure["WallSpec"]]["inputMethod"] == "断熱材種類を入力":
                ...
                if wall_configure["Direction"] == "水平（上）":  # 天井と見なす。
                    ...
                    inputdata["EnvelopeSet"][room_zone_name]["WallList"][wall_id]["UA_wall"] = \
                        inputdata["WallConfigure"][wall_configure["WallSpec"]]["Uvalue_roof"] * wall_configure[
                            "WallArea"]
                    ...
                    # 外壁のUA（熱貫流率×面積）を計算
                    inputdata["EnvelopeSet"][room_zone_name]["WallList"][wall_id]["UA_wall"] = \
                        inputdata["WallConfigure"][wall_configure["WallSpec"]]["Uvalue_floor"] * wall_configure[
                            "WallArea"]
                    ...
                    # 外壁のUA（熱貫流率×面積）を計算
                    inputdata["EnvelopeSet"][room_zone_name]["WallList"][wall_id]["UA_wall"] = \
                        inputdata["WallConfigure"][wall_configure["WallSpec"]]["Uvalue_wall"] * wall_configure[
                            "WallArea"]
            ...
            for (wall_id, wall_configure) in enumerate(inputdata["EnvelopeSet"][room_zone_name]["WallList"]):
                ...
                for (window_id, window_configure) in enumerate(wall_configure["WindowList"]):
                    ...
                    if window_configure["WindowID"] != "無":  # 窓がある場合
                        ...
                        if wall_configure["WallType"] == "日の当たる外壁" or wall_configure[
                            "WallType"] == "地盤に接する外壁_Ver2":
                            ...
                            ## ③ 夜間放射による熱取得（マイナス）
                            if wall_configure["Direction"] == "水平（上）" or wall_configure["Direction"] == "水平（下）":
                                Qwind_N = Qwind_N - window_configure["UA_window"] * 0.9 * 0.04 * \
                                          solor_radiation["夜間"]["水平"]
                            else:
                                Qwind_N = Qwind_N - window_configure["UA_window"] * 0.9 * 0.04 * \
                                          solor_radiation["夜間"]["垂直"]
                            ...
                            ## ③ 夜間放射による熱取得（マイナス）
                            Qwind_N = Qwind_N - window_configure["UA_window"] * 0.9 * 0.04 * solor_radiation["夜間"][
                                "水平"]

        #  室面積あたりの熱量に変換 [Wh/m2/日]
        resultJson["Qroom"][room_zone_name]["Qwall_T"] = Qwall_T / inputdata["AirConditioningZone"][room_zone_name][
            "zoneArea"]
        resultJson["Qroom"][room_zone_name]["Qwall_S"] = Qwall_S / inputdata["AirConditioningZone"][room_zone_name][
            "zoneArea"]
        resultJson["Qroom"][room_zone_name]["Qwall_N"] = Qwall_N / inputdata["AirConditioningZone"][room_zone_name][
            "zoneArea"]
        resultJson["Qroom"][room_zone_name]["Qwind_T"] = Qwind_T / inputdata["AirConditioningZone"][room_zone_name][
            "zoneArea"]
        resultJson["Qroom"][room_zone_name]["Qwind_S"] = Qwind_S / inputdata["AirConditioningZone"][room_zone_name][
            "zoneArea"]
        resultJson["Qroom"][room_zone_name]["Qwind_N"] = Qwind_N / inputdata["AirConditioningZone"][room_zone_name][
            "zoneArea"]
    ...
    ## 室負荷計算のための係数（解説書 A.3）
    with open(database_directory + 'QROOM_COEFFI_AREA' + inputdata["Building"]["Region"] + '.json', 'r',
              encoding='utf-8') as f:
    ...
    for room_zone_name in inputdata["AirConditioningZone"]:
            ...
            if room_usage[dd] > 0:
                ...
                if ac_mode[dd] == "冷房":
                    ...
                    Qroom_CTC[dd] = QROOM_COEFFI[btype][rtype][onoff]["冷房期"]["外気温変動"]["冷房負荷"]["係数"] * \
                                    (resultJson["Qroom"][room_zone_name]["Qwall_T"][dd] +
                                     resultJson["Qroom"][room_zone_name]["Qwall_N"][dd] + \
                                     resultJson["Qroom"][room_zone_name]["Qwind_T"][dd] +
                                     resultJson["Qroom"][room_zone_name]["Qwind_N"][dd]) + \
                    ...
                    Qroom_CSR[dd] = QROOM_COEFFI[btype][rtype][onoff]["冷房期"]["日射量変動"]["冷房負荷"]["係数"] * \
                                    (resultJson["Qroom"][room_zone_name]["Qwall_S"][dd] +
                                     resultJson["Qroom"][room_zone_name]["Qwind_S"][dd]) + \
        ...
        resultJson["Qroom"][room_zone_name]["QroomDc"] = Qcool * (3600 / 1000000) * \
                                                         inputdata["AirConditioningZone"][room_zone_name]["zoneArea"]
        resultJson["Qroom"][room_zone_name]["QroomDh"] = Qheat * (3600 / 1000000) * \
                                                         inputdata["AirConditioningZone"][room_zone_name]["zoneArea"]
                ...
                if room_zone_name in resultJson["Qroom"]:  # SP-4シートに入力された室が空調ゾーンとして存在していれば
                    ...
                    if "QroomDc" in inputdata["SpecialInputData"]["Qroom"][room_zone_name]:
                        resultJson["Qroom"][room_zone_name]["QroomDc"] = \
                            inputdata["SpecialInputData"]["Qroom"][room_zone_name]["QroomDc"]

                    # 室負荷（暖房要求）の置き換え
                    if "QroomDh" in inputdata["SpecialInputData"]["Qroom"][room_zone_name]:
                        resultJson["Qroom"][room_zone_name]["QroomDh"] = \
                            inputdata["SpecialInputData"]["Qroom"][room_zone_name]["QroomDh"]
            ...
            for hh in range(0, 24):
                if roomScheduleRoom[room_zone_name][dd][hh] > 0:
                    ...
                    resultJson["Qroom"][room_zone_name]["QroomHc"][dd][hh] = \
                        resultJson["Qroom"][room_zone_name]["QroomDc"][dd] / daily_opetime
                    # 暖房熱取得
                    resultJson["Qroom"][room_zone_name]["QroomHh"][dd][hh] = \
                        resultJson["Qroom"][room_zone_name]["QroomDh"][dd] / daily_opetime
    ...
    if False:
        ...
        for room_zone_name in inputdata["AirConditioningZone"]:
            ...
            input_heatcalc["rooms"][0]["schedule"]["heat_generation_lighting"] = np.reshape(
                Heat_light_hourly[room_zone_name], 8760) * inputdata["AirConditioningZone"][room_zone_name]["zoneArea"]
            # 機器発熱スケジュール[W]
            input_heatcalc["rooms"][0]["schedule"]["heat_generation_appliances"] = np.reshape(
                Heat_OAapp_hourly[room_zone_name], 8760) * inputdata["AirConditioningZone"][room_zone_name]["zoneArea"]
            # 人員数[人]
            input_heatcalc["rooms"][0]["schedule"]["number_of_people"] = np.reshape(
                Num_of_Person_hourly[room_zone_name], 8760) * inputdata["AirConditioningZone"][room_zone_name][
                                                                             "zoneArea"]
            ...
            if room_zone_name in inputdata["EnvelopeSet"]:
                ...
                for (wall_id, wall_configure) in enumerate(inputdata["EnvelopeSet"][room_zone_name]["WallList"]):
                    ...
                    if inputdata["EnvelopeSet"][room_zone_name]["WallList"][wall_id]["WallType"] == "日の当たる外壁":
                        ...
                        is_sun_striked_outside = False
                    elif inputdata["EnvelopeSet"][room_zone_name]["WallList"][wall_id][
                        "WallType"] == "地盤に接する外壁_Ver2":
                        boundary_type = "ground"
                        is_sun_striked_outside = False
            ...
            for dd in range(0, 365):
                for hh in range(0, 24):
                    ...
                    if heatload[dd][hh] > 0:
                        # 暖房負荷 [W] → [MJ/hour]
                        resultJson["Qroom"][room_zone_name]["QroomHh"][dd][hh] = (-1) * heatload[dd][
                            hh] * 3600 / 1000000
                        # 暖房負荷 [W] → [MJ/day]
                        ...
                        # 冷房負荷 [W] → [MJ/hour]
                        resultJson["Qroom"][room_zone_name]["QroomHc"][dd][hh] = (-1) * heatload[dd][
                            hh] * 3600 / 1000000
                        # 冷房負荷 [W]→ [MJ/day]
                        resultJson["Qroom"][room_zone_name]["QroomDc"][dd] += (-1) * heatload[dd][hh] * 3600 / 1000000

            print(
                f'室負荷（冷房要求）の合計 heatload_for_cooling: {np.sum(resultJson["Qroom"][room_zone_name]["QroomDc"], 0)}')
            print(
                f'室負荷（暖房要求）の合計 heatload_for_heating: {np.sum(resultJson["Qroom"][room_zone_name]["QroomDh"], 0)}')
    ...
    for room_zone_name in inputdata["AirConditioningZone"]:
        # 結果の集約 [MJ/年]
        resultJson["Qroom"][room_zone_name]["建物用途"] = inputdata["AirConditioningZone"][room_zone_name][
            "buildingType"]
    ...
    for ahu_name in inputdata["AirHandlingSystem"]:
        ...
        if "終日" in resultJson["AHU"][ahu_name]["day_mode"]:  # 一つでも「終日」があれば
            ...
            resultJson["AHU"][ahu_name]["HoaDayAve"] = Hoa_ave
                ...
                resultJson["AHU"][ahu_name]["day_mode"]):  # 全て「昼」であれば
            ...
            resultJson["AHU"][ahu_name]["HoaDayAve"] = Hoa_day
                ...
                resultJson["AHU"][ahu_name]["day_mode"]):  # 全て夜であれば
        ...
        for dd in range(0, 365):
            ...
            if resultJson["AHU"][ahu_name]["Tahu_total"][dd] == 0:
                ...
                if (resultJson["AHU"][ahu_name]["Qroom"]["cooling_for_room"][dd] == 0) and \
                        (resultJson["AHU"][ahu_name]["Qroom"]["heating_for_room"][
                             dd] == 0):  # 外調機を想定（空調運転時間は0より大きいが、Qroomが0である場合）
                    ...
                    if abs(resultJson["AHU"][ahu_name]["Qroom"]["cooling_for_room"][dd]) < abs(
                            resultJson["AHU"][ahu_name]["Qroom"]["heating_for_room"][dd]):

                        # 暖房負荷の方が大きい場合
                        ratio = abs(resultJson["AHU"][ahu_name]["Qroom"]["cooling_for_room"][dd]) / \
                                (abs(resultJson["AHU"][ahu_name]["Qroom"]["cooling_for_room"][dd]) + abs(
                                    resultJson["AHU"][ahu_name]["Qroom"]["heating_for_room"][dd]))

                        resultJson["AHU"][ahu_name]["Tahu"]["cooling_for_room"][dd] = math.ceil(
                        ...
                        resultJson["AHU"][ahu_name]["Tahu"]["heating_for_room"][dd] = math.ceil(
                            resultJson["AHU"][ahu_name]["Tahu_total"][dd] * ratio)
                        resultJson["AHU"][ahu_name]["Tahu"]["cooling_for_room"][dd] = \
                            resultJson["AHU"][ahu_name]["Tahu_total"][dd] - \
                            resultJson["AHU"][ahu_name]["Tahu"]["heating_for_room"][dd]
            ...
            if resultJson["AHU"][ahu_name]["Tahu_total"][dd] > 0:  # 空調機が稼働する場合
                ...
                if ac_mode[dd] == "暖房":
                    ...
                    if inputdata["AirHandlingSystem"][ahu_name]["isAirHeatExchanger"] == "無":  # 全熱交換器がない場合
                        ...
                        if (resultJson["AHU"][ahu_name]["HoaDayAve"][dd] > Hroom[dd]) and (
                                inputdata["AirHandlingSystem"][ahu_name]["AirHeatExchangerControl"] == "有"):
        ...
        for dd in range(0, 365):
            ...
            if resultJson["AHU"][ahu_name]["Tahu_total"][dd] > 0:  # 空調機が稼働する場合
                ...
                if (inputdata["AirHandlingSystem"][ahu_name]["isEconomizer"] == "有") and (
                        resultJson["AHU"][ahu_name]["Tahu"]["cooling_for_room"][dd] > 0):  # 外気冷房があり、室負荷が冷房要求であれば

                    # 外気冷房運転時の外気風量 [kg/s]
                    resultJson["AHU"][ahu_name]["Economizer"]["AHUVovc"][dd] = \
                        resultJson["AHU"][ahu_name]["Qroom"]["cooling_for_room"][dd] / \
                        ((Hroom[dd] - resultJson["AHU"][ahu_name]["HoaDayAve"][dd]) * (3600 / 1000) *
                         resultJson["AHU"][ahu_name]["Tahu"]["cooling_for_room"][dd])

                    # 上限・下限
                    if resultJson["AHU"][ahu_name]["Economizer"]["AHUVovc"][dd] < \
                            inputdata["AirHandlingSystem"][ahu_name]["outdoorAirVolume_cooling"] * 1.293 / 3600:
                    ...
                    elif resultJson["AHU"][ahu_name]["Economizer"]["AHUVovc"][dd] > \
            ...
            if (resultJson["AHU"][ahu_name]["Tahu"]["cooling_for_room"][dd] == 0) and (
                ...
                if (inputdata["AirHandlingSystem"][ahu_name]["isOutdoorAirCut"] == "無"):
                    ...
                    resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd] = \
                        resultJson["AHU"][ahu_name]["qoaAHU"][dd] * resultJson["AHU"][ahu_name]["Tahu_total"][
                            dd] * 3600 / 1000
                    ...
                    if resultJson["AHU"][ahu_name]["Tahu_total"][dd] > 1:
                        resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd] = \
                            resultJson["AHU"][ahu_name]["qoaAHU"][dd] * (
                                    resultJson["AHU"][ahu_name]["Tahu_total"][dd] - 1) * 3600 / 1000
                ...
                if resultJson["AHU"][ahu_name]["Tahu"]["cooling_for_room"][dd] > 0:
                    ...
                    if (inputdata["AirHandlingSystem"][ahu_name]["isOutdoorAirCut"] == "有") and \
                        ...
                        resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd] = \
                            resultJson["AHU"][ahu_name]["Qroom"]["cooling_for_room"][dd] + \
                            resultJson["AHU"][ahu_name]["qoaAHU"][dd] * (
                                    resultJson["AHU"][ahu_name]["Tahu"]["cooling_for_room"][dd] - 1) * 3600 / 1000
                        ...
                        resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd] = \
                            resultJson["AHU"][ahu_name]["Qroom"]["cooling_for_room"][dd] + \
                            resultJson["AHU"][ahu_name]["qoaAHU"][dd] * (
                                resultJson["AHU"][ahu_name]["Tahu"]["cooling_for_room"][dd]) * 3600 / 1000
                ...
                if resultJson["AHU"][ahu_name]["Tahu"]["heating_for_room"][dd] > 0:
                    ...
                    if (inputdata["AirHandlingSystem"][ahu_name]["isOutdoorAirCut"] == "有") and \
                        ...
                        resultJson["AHU"][ahu_name]["Qahu"]["heating_for_room"][dd] = \
                            resultJson["AHU"][ahu_name]["Qroom"]["heating_for_room"][dd] + \
                            resultJson["AHU"][ahu_name]["qoaAHU"][dd] * (
                                    resultJson["AHU"][ahu_name]["Tahu"]["heating_for_room"][dd] - 1) * 3600 / 1000
                        ...
                        resultJson["AHU"][ahu_name]["Qahu"]["heating_for_room"][dd] = \
                            resultJson["AHU"][ahu_name]["Qroom"]["heating_for_room"][dd] + \
                            resultJson["AHU"][ahu_name]["qoaAHU"][dd] * (
                                resultJson["AHU"][ahu_name]["Tahu"]["heating_for_room"][dd]) * 3600 / 1000
    ...
    if debug:  # pragma: no cover
        ...
        for ahu_name in inputdata["AirHandlingSystem"]:
            ...
            print(
                f'外気導入量 outdoorAirVolume_cooling {inputdata["AirHandlingSystem"][ahu_name]["outdoorAirVolume_cooling"]} m3/h')
            print(
                f'外気冷房時最大風量 EconomizerMaxAirVolume {inputdata["AirHandlingSystem"][ahu_name]["EconomizerMaxAirVolume"]} m3/h')
            ...
            print(
                f'室負荷が正（冷房要求）であるときの空調機群の運転時間 Tahu： {np.sum(resultJson["AHU"][ahu_name]["Tahu"]["cooling_for_room"], 0)} 時間')
            print(
                f'室負荷が負（暖房要求）であるときの空調機群の運転時間 Tahu： {np.sum(resultJson["AHU"][ahu_name]["Tahu"]["heating_for_room"], 0)} 時間')
            print(
                f'室負荷が正（冷房要求）であるときの空調負荷 Qahu： {np.sum(resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"], 0)}')
            print(
                f'室負荷が負（暖房要求）であるときの空調負荷 Qahu： {np.sum(resultJson["AHU"][ahu_name]["Qahu"]["heating_for_room"], 0)}')
                    ...
                    for dd in range(0, 365):
                        for hh in range(0, 24):
                            if inputdata["SpecialInputData"]["Qahu"][ahu_name][dd][hh] > 0:  # 冷房負荷であれば
                                ...
                                # 空調負荷[kW] → [MJ/h]
                                Qahu_cooling[dd] += inputdata["SpecialInputData"]["Qahu"][ahu_name][dd][
                                                        hh] * 3600 / 1000
                                Tahu_cooling[dd] += 1
                                ...
                                # 空調負荷[kW] → [MJ/h]
                                Qahu_heating[dd] += inputdata["SpecialInputData"]["Qahu"][ahu_name][dd][
                                                        hh] * 3600 / 1000
                                Tahu_heating[dd] += 1
    ...
    for ahu_name in inputdata["AirHandlingSystem"]:
        ...
        for requirement_type in ["cooling_for_room", "heating_for_room"]:
            ...
            if requirement_type == "cooling_for_room":  # 室負荷が正（冷房要求）であるとき
                ...
                for dd in range(0, 365):
                    ...
                    if resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd] >= 0 and \
                        ...
                        La[dd] = (resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd] /
                                  resultJson["AHU"][ahu_name]["Tahu"]["cooling_for_room"][dd] * 1000 / 3600) / \
                                 inputdata["AirHandlingSystem"][ahu_name]["RatedCapacityCooling"]
                ...
                for dd in range(0, 365):
                    ...
                    if resultJson["AHU"][ahu_name]["Qahu"]["heating_for_room"][dd] <= 0 and \
                        ...
                        # 空調負荷が負（加熱コイル負荷）である場合　→　定格加熱能力で除して負荷率を求める。
                        La[dd] = (resultJson["AHU"][ahu_name]["Qahu"]["heating_for_room"][dd] /
                                  resultJson["AHU"][ahu_name]["Tahu"]["heating_for_room"][dd] * 1000 / 3600) / \
                                 inputdata["AirHandlingSystem"][ahu_name]["RatedCapacityHeating"]
            ...
            if (inputdata["AirHandlingSystem"][ahu_name]["RatedCapacityCooling"] > 0) or (
                    inputdata["AirHandlingSystem"][ahu_name]["RatedCapacityHeating"] > 0):
    ...
    if debug:  # pragma: no cover
        ...
        for ahu_name in inputdata["AirHandlingSystem"]:
            ...
            for dd in range(0, 365):
                matlix_AHUc_L[int(resultJson["AHU"][ahu_name]["LdAHUc"]["cooling_for_room"][dd] - 1)] += \
                    resultJson["AHU"][ahu_name]["TdAHUc"]["cooling_for_room"][dd]
                matlix_AHUc_L[int(resultJson["AHU"][ahu_name]["LdAHUc"]["heating_for_room"][dd] - 1)] += \
                    resultJson["AHU"][ahu_name]["TdAHUc"]["heating_for_room"][dd]
                matlix_AHUh_L[int(resultJson["AHU"][ahu_name]["LdAHUh"]["cooling_for_room"][dd] - 1)] += \
                    resultJson["AHU"][ahu_name]["TdAHUh"]["cooling_for_room"][dd]
                matlix_AHUh_L[int(resultJson["AHU"][ahu_name]["LdAHUh"]["heating_for_room"][dd] - 1)] += \
                    resultJson["AHU"][ahu_name]["TdAHUh"]["heating_for_room"][dd]
    ...
    for ahu_name in inputdata["AirHandlingSystem"]:
        ...
        for unit_id, unit_configure in enumerate(inputdata["AirHandlingSystem"][ahu_name]["AirHandlingUnit"]):
            ...
            # 初期化
            inputdata["AirHandlingSystem"][ahu_name]["AirHandlingUnit"][unit_id]["energy_consumption_ratio"] = np.ones(
                len(aveL))
            ...
            for iL in range(0, len(aveL)):
                if aveL[iL] > 1:
                    inputdata["AirHandlingSystem"][ahu_name]["AirHandlingUnit"][unit_id]["energy_consumption_ratio"][
                        iL] = 1.2
                elif aveL[iL] == 0:
                    inputdata["AirHandlingSystem"][ahu_name]["AirHandlingUnit"][unit_id]["energy_consumption_ratio"][
                        iL] = 0
    ...
    if debug:  # pragma: no cover
        ...
        for ahu_name in inputdata["AirHandlingSystem"]:
            ...
            for unit_id, unit_configure in enumerate(inputdata["AirHandlingSystem"][ahu_name]["AirHandlingUnit"]):
                ...
                print(
                    f'負荷率帯毎のエネルギー消費量 energy_consumption_ratio {inputdata["AirHandlingSystem"][ahu_name]["AirHandlingUnit"][unit_id]["energy_consumption_ratio"]}')
    ...
    for ahu_name in inputdata["AirHandlingSystem"]:
        ...
        for unit_id, unit_configure in enumerate(inputdata["AirHandlingSystem"][ahu_name]["AirHandlingUnit"]):
            ...
            if debug:  # pragma: no cover
                print(f'--- 空調機群名 {ahu_name} ---')
                print(
                    f'送風機単体の定格消費電力: {inputdata["AirHandlingSystem"][ahu_name]["AirHandlingUnit"][unit_id]["FanPowerConsumption_total"]}')
    ...
    for ahu_name in inputdata["AirHandlingSystem"]:
        for dd in range(0, 365):
            ...
            if resultJson["AHU"][ahu_name]["LdAHUc"]["cooling_for_room"][dd] > 0:  # 空調負荷が正（冷却コイル負荷）である場合
                ...
                # 運転時間の合計 h
                resultJson["AHU"][ahu_name]["TdAHUc_total"][dd] += \
                    resultJson["AHU"][ahu_name]["TdAHUc"]["cooling_for_room"][dd]
                ...
                # 運転時間の合計 h
                resultJson["AHU"][ahu_name]["TdAHUh_total"][dd] += \
                    resultJson["AHU"][ahu_name]["TdAHUh"]["cooling_for_room"][dd]
    ...
    for ahu_name in inputdata["AirHandlingSystem"]:
        ...
        resultJson["年間エネルギー消費量"]["空調機群全熱交換器[MWh]"] += np.sum(resultJson["AHU"][ahu_name]["E_AHUaex_day"],
                                                                           0)
    ...
    resultJson["年間エネルギー消費量"]["空調機群ファン[GJ]"] = resultJson["年間エネルギー消費量"][
                                                           "空調機群ファン[MWh]"] * bc.fprime / 1000
    resultJson["年間エネルギー消費量"]["空調機群全熱交換器[GJ]"] = resultJson["年間エネルギー消費量"][
    ...
    for ahu_name in inputdata["AirHandlingSystem"]:
        ...
        resultJson["AHU"][ahu_name]["定格能力（冷房）[kW]"] = inputdata["AirHandlingSystem"][ahu_name][
            "RatedCapacityCooling"]
        resultJson["AHU"][ahu_name]["定格能力（暖房）[kW]"] = inputdata["AirHandlingSystem"][ahu_name][
            "RatedCapacityHeating"]
        resultJson["AHU"][ahu_name]["定格消費電力[kW]"] = inputdata["AirHandlingSystem"][ahu_name][
        ...
        resultJson["AHU"][ahu_name]["年間空調時間（冷房）[時間]"] = \
            np.sum(resultJson["AHU"][ahu_name]["TdAHUc"]["cooling_for_room"]) + np.sum(
                resultJson["AHU"][ahu_name]["TdAHUc"]["heating_for_room"])
        resultJson["AHU"][ahu_name]["年間空調時間（暖房）[時間]"] = \
            np.sum(resultJson["AHU"][ahu_name]["TdAHUh"]["cooling_for_room"]) + np.sum(
                resultJson["AHU"][ahu_name]["TdAHUh"]["heating_for_room"])
        ...
        if resultJson["AHU"][ahu_name]["定格能力（冷房）[kW]"] != 0:
            resultJson["AHU"][ahu_name]["平均負荷率（冷房）[-]"] = \
                resultJson["AHU"][ahu_name]["平均空調負荷（冷房）[kW]"] / resultJson["AHU"][ahu_name][
                    "定格能力（冷房）[kW]"]
        else:
        ...
        if resultJson["AHU"][ahu_name]["定格能力（暖房）[kW]"] != 0:
            resultJson["AHU"][ahu_name]["平均負荷率（暖房）[-]"] = \
                resultJson["AHU"][ahu_name]["平均空調負荷（暖房）[kW]"] / resultJson["AHU"][ahu_name][
                    "定格能力（暖房）[kW]"]
        else:
    ...
    for pump_name in inputdata["PUMP"]:
        ...
        for unit_id, unit_configure in enumerate(inputdata["PUMP"][pump_name]["SecondaryPump"]):
            ...
            inputdata["PUMP"][pump_name]["Vpsr"] += inputdata["PUMP"][pump_name]["SecondaryPump"][unit_id][
    ...
    for ahu_name in inputdata["AirHandlingSystem"]:
        ...
        for dd in range(0, 365):
            ...
            if ac_mode[dd] == "暖房":  ## 暖房期である場合
                ...
                if (resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd] > 0) and \
                        (inputdata["AirHandlingSystem"][ahu_name]["isSimultaneousSupply_heating"] == "無"):
                    resultJson["AHU"][ahu_name]["Qahu_remainC"][dd] += (
                        resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd])
        ...
        for ahu_name in inputdata["PUMP"][pump_name]["AHU_list"]:
            ...
            for dd in range(0, 365):
                ...
                if inputdata["PUMP"][pump_name]["mode"] == "cooling":  # 冷水ポンプの場合
                    ...
                    # 室負荷が冷房要求である場合において空調負荷が正である場合
                    if resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd] > 0:
                        if resultJson["AHU"][ahu_name]["Economizer"]["Qahu_oac"][
                            dd] > 0:  # 外冷時はファン発熱量足さない ⇒ 小さな負荷が出てしまう
                            if abs(resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd] -
                                   resultJson["AHU"][ahu_name]["Economizer"]["Qahu_oac"][dd]) < 1:
                    ...
                    if resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd] < 0:
                        resultJson["PUMP"][pump_name]["Qps"][dd] += \
                            (-1) * (resultJson["AHU"][ahu_name]["Qahu"]["cooling_for_room"][dd] +
                                    resultJson["PUMP"][pump_name]["Qpsahu_fan"][dd])

                    # 室負荷が暖房要求である場合において空調負荷が正である場合
                    if resultJson["AHU"][ahu_name]["Qahu"]["heating_for_room"][dd] < 0:
                        resultJson["PUMP"][pump_name]["Qps"][dd] += \
                            (-1) * (resultJson["AHU"][ahu_name]["Qahu"]["heating_for_room"][dd] +
                                    resultJson["PUMP"][pump_name]["Qpsahu_fan"][dd])
        ...
        for unit_id, unit_configure in enumerate(inputdata["PUMP"][pump_name]["SecondaryPump"]):
            ...
            inputdata["PUMP"][pump_name]["SecondaryPump"][unit_id]["Qpsr"] = \
                inputdata["PUMP"][pump_name]["TemperatureDifference"] * unit_configure[
                    "RatedWaterFlowRate_total"] * 4.1860 * 1000 / 3600
        ...
        if inputdata["PUMP"][pump_name]["Qpsr"] != 0:  # 仮想ポンプ（二次ポンプがないシステム用の仮想ポンプ）は除く
            ...
            for dd in range(0, 365):
                ...
                if resultJson["PUMP"][pump_name]["Tps"][dd] > 0:
                    ...
                    Lpump[dd] = (resultJson["PUMP"][pump_name]["Qps"][dd] / resultJson["PUMP"][pump_name]["Tps"][
                        dd] * 1000 / 3600) \
            ...
            for dd in range(0, 365):
                ...
                if (resultJson["PUMP"][pump_name]["Tps"][dd] > 0) and (
                        inputdata["PUMP"][pump_name]["Qpsr"] > 0):  # ゼロ割でNaNになっている値を飛ばす
                ...
                for iL in range(0, divL):
                    ...
                    for rr in range(0, int(MxPUMPNum[iL])):
                        ...
                        if (inputdata["PUMP"][pump_name]["SecondaryPump"][rr]["ContolType"] == "無") or \
                            ...
                            if aveL[iL] > 1.0:
                                MxPUMPPower[iL] += inputdata["PUMP"][pump_name]["SecondaryPump"][rr][
                                                       "RatedPowerConsumption_total"] * PUMPvwvfac[iL]
                            else:
                                MxPUMPPower[iL] += inputdata["PUMP"][pump_name]["SecondaryPump"][rr][
                                                       "RatedPowerConsumption_total"] * PUMPvwvfac[iL]
                            ...
                            # 変流量ポンプjの負荷率 [-]
                            tmpL = ((Qpsr_iL - Qtmp_CWV) / numVWV) / inputdata["PUMP"][pump_name]["SecondaryPump"][rr][
                                "Qpsr"]
            ...
            if resultJson["PUMP"][pump_name]["TdPUMP"][dd] > 0:
                resultJson["PUMP"][pump_name]["E_pump_day"][dd] = \
                    resultJson["PUMP"][pump_name]["MxPUMPPower"][
                        int(resultJson["PUMP"][pump_name]["LdPUMP"][dd]) - 1] / 1000 * \
    ...
    for pump_name in inputdata["PUMP"]:
        ...
        if pump_name.startswith("dummyPump") == False:
            ...
            resultJson["PUMP"][pump_name]["定格消費電力[kW]"] = inputdata["PUMP"][pump_name][
            ...
            resultJson["PUMP"][pump_name]["平均負荷率[-]"] = resultJson["PUMP"][pump_name]["平均処理熱量[kW]"] / \
    ...
    for ref_name in inputdata["REF"]:
        ...
        for dd in range(0, 365):
            ...
            if resultJson["REF"][ref_name]["Tref"][dd] == 0:
                ...
                resultJson["REF"][ref_name]["Qref_kW"][dd] = resultJson["REF"][ref_name]["Qref"][dd] / \
                                                             resultJson["REF"][ref_name]["Tref"][dd] * 1000 / 3600
    ...
    for ref_name in inputdata["REF"]:
        ...
        for unit_id, unit_configure in enumerate(inputdata["REF"][ref_name]["Heatsource"]):
            ...
            if fuel_type == "電力":
                ...
                # 熱源群ごとに積算
            inputdata["REF"][ref_name]["Eref_rated_primary"] += inputdata["REF"][ref_name]["Heatsource"][unit_id][
                "Eref_rated_primary"]
    ...
    for ref_name in inputdata["REF"]:
        ...
        for dd in range(0, 365):
            ...
            if resultJson["REF"][ref_name]["Lref"][dd] > 0:
                ...
                # 負荷率帯マトリックス
                resultJson["REF"][ref_name]["matrix_iL"][dd] = count_Matrix(resultJson["REF"][ref_name]["Lref"][dd],
                                                                            mxL)
        ...
        if inputdata["REF"][ref_name]["isStorage"] == "蓄熱":
            ...
            for dd in range(0, 365):
                ...
                if resultJson["REF"][ref_name]["matrix_iT"][dd] > 1:
                    resultJson["REF"][ref_name]["matrix_iT"][dd] = resultJson["REF"][ref_name]["matrix_iT"][
                                                                       dd] - 1  # 外気温帯を1つ下げる。
    ...
    ToawbC = Area[inputdata["Building"]["Region"] + "地域"]["湿球温度係数_冷房a1"] * ToadbC + \
             Area[inputdata["Building"]["Region"] + "地域"]["湿球温度係数_冷房a0"]
    ToawbH = Area[inputdata["Building"]["Region"] + "地域"]["湿球温度係数_暖房a1"] * ToadbH + \
            ...
            if "地盤A" in unit_configure["parameter"]["熱源種類"] or "地盤B" in unit_configure["parameter"][
                ...
                for dd in range(365):
                    ...
                    theta_wo_m = AC_gshp_openloop["theta_ac_wo_ave"][inputdata["Building"]["Region"] + "地域"] + \
                                 AC_gshp_openloop["theta_ac_wo_m"][inputdata["Building"]["Region"] + "地域"][
                                     bc.day2month(dd)]
                ...
                elif unit_configure["parameter"]["熱源種類"] == "不要" and inputdata["REF"][ref_name][
                    "mode"] == "cooling":
                    inputdata["REF"][ref_name]["Heatsource"][unit_id]["matrix_T"] = ToadbC  # 乾球温度
                    ...
                    for gound_type in range(1, 6):
                        ...
                        if unit_configure["parameter"]["熱源種類"] == "地盤" + str(int(gound_type)) and \
                            ...
                            inputdata["REF"][ref_name]["Heatsource"][unit_id]["matrix_T"] = \
                                ...
                                (ghspToa_ave[iAREA] + gshp_ac[igsType] * resultJson["REF"][ref_name]["ghsp_Rq"] +
                                 gshp_bc[igsType])
                            ...
                            inputdata["REF"][ref_name]["Heatsource"][unit_id]["matrix_T"] = \
                                ...
                                (ghspToa_ave[iAREA] + gshp_ah[igsType] * resultJson["REF"][ref_name]["ghsp_Rq"] +
                                 gshp_bh[igsType])
                ...
                for ref_name in [ref_original_name + "_冷房", ref_original_name + "_暖房",
                    ...
                    if ref_name in inputdata["REF"]:
                        for unit_id, unit_configure in enumerate(inputdata["REF"][ref_name]["Heatsource"]):
                            for dd in range(0, 365):
                                inputdata["REF"][ref_name]["Heatsource"][unit_id]["heatsource_temperature"][dd] = \
                                    inputdata["SpecialInputData"]["heatsource_temperature_monthly"][ref_original_name][
                                        bc.day2month(dd)]
            ...
            for unit_id in range(0, int(resultJson["REF"][ref_name]["num_of_operation"][dd])):
                ...
                if len(inputdata["REF"][ref_name]["Heatsource"][unit_id]["parameter"][
                    ...
                    for para_id in range(0, len(
                        ...
                        if inputdata["REF"][ref_name]["Heatsource"][unit_id]["heatsource_temperature"][dd] > \
                                inputdata["REF"][ref_name]["Heatsource"][unit_id]["parameter"]["部分負荷特性"][para_id][
                                    "冷却水温度下限"] and \
                                inputdata["REF"][ref_name]["Heatsource"][unit_id]["heatsource_temperature"][dd] <= \
                                inputdata["REF"][ref_name]["Heatsource"][unit_id]["parameter"]["部分負荷特性"][para_id][
                                    "冷却水温度上限"]:
                ...
                tmpL = 0
                if resultJson["REF"][ref_name]["load_ratio"][dd] < \
                        inputdata["REF"][ref_name]["Heatsource"][unit_id]["parameter"]["部分負荷特性"][xCurveNum][
                            "下限"]:
                    tmpL = inputdata["REF"][ref_name]["Heatsource"][unit_id]["parameter"]["部分負荷特性"][xCurveNum][
                        "下限"]
                elif resultJson["REF"][ref_name]["load_ratio"][dd] > \
                        inputdata["REF"][ref_name]["Heatsource"][unit_id]["parameter"]["部分負荷特性"][xCurveNum][
                            "上限"]:
                    tmpL = inputdata["REF"][ref_name]["Heatsource"][unit_id]["parameter"]["部分負荷特性"][xCurveNum][
                        "上限"]
            ...
            if inputdata["REF"][ref_name]["checkGEGHP"] == 1:
                ...
                for unit_id in range(0, int(resultJson["REF"][ref_name]["num_of_operation"][dd])):
                    ...
                    if "消費電力自給装置" in inputdata["REF"][ref_name]["Heatsource"][unit_id]["HeatsourceType"]:
                        ...
                        if aveLperU <= 0.3:
                            resultJson["REF"][ref_name]["E_ref_sub"][dd] += 0.3 * \
                                                                            inputdata["REF"][ref_name]["Heatsource"][
                                                                                unit_id][
                                                                                "Heatsource_sub_RatedPowerConsumption_total"]
                        else:
                            resultJson["REF"][ref_name]["E_ref_sub"][dd] += aveLperU * \
                                                                            inputdata["REF"][ref_name]["Heatsource"][
                                                                                unit_id][
                                                                                "Heatsource_sub_RatedPowerConsumption_total"]
            ...
            if resultJson["REF"][ref_name]["Tref"][dd] == 0:
                ...
                for unit_id in range(0, len(inputdata["REF"][ref_name]["Heatsource"])):
                    ...
                    resultJson["REF"][ref_name]["E_ref_day"][dd] += \
                        resultJson["REF"][ref_name]["Heatsource"][unit_id]["E_ref_main"][dd] * 3600 / 1000 * \
                        resultJson["REF"][ref_name]["Tref"][dd]
                    ...
                    if inputdata["REF"][ref_name]["Heatsource"][unit_id][
                        ...
                        resultJson["REF"][ref_name]["E_ref_day_MWh"][dd] += \
                            resultJson["REF"][ref_name]["Heatsource"][unit_id]["E_ref_main"][dd] * 3600 / 1000 * \
                            resultJson["REF"][ref_name]["Tref"][dd] / bc.fprime
    ...
    for ref_name in inputdata["REF"]:
        ...
        resultJson["REF"][ref_name]["熱源群熱源補機[GJ]"] = resultJson["REF"][ref_name][
                                                                "熱源群熱源補機[MWh]"] * bc.fprime / 1000
        resultJson["REF"][ref_name]["熱源群一次ポンプ[GJ]"] = resultJson["REF"][ref_name][
                                                               "熱源群一次ポンプ[MWh]"] * bc.fprime / 1000
        resultJson["REF"][ref_name]["熱源群冷却塔ファン[GJ]"] = resultJson["REF"][ref_name][
                                                                 "熱源群冷却塔ファン[MWh]"] * bc.fprime / 1000
        resultJson["REF"][ref_name]["熱源群冷却水ポンプ[GJ]"] = resultJson["REF"][ref_name][
 ...
 \
                resultJson["REF"][ref_name]["年間平均負荷率[-]"] = \
                    (resultJson["REF"][ref_name]["年積算熱源負荷[GJ]"] * 1000000 / (
    ...
    if len(inputdata["CogenerationSystems"]) == 1:  # コジェネがあれば実行
        ...
        # 熱源主機の電力消費量 [MWh/day]
        resultJson["for_CGS"]["E_ref_main_MWh_day"] = resultJson["日別エネルギー消費量"][
        ...
        for ref_name in inputdata["REF"]:
            ...
            if ref_name == resultJson["for_CGS"]["CGS_refName_C"]:
                ...
                for unit_id, unit_configure in enumerate(inputdata["REF"][ref_name]["Heatsource"]):
                    ...
                    if unit_configure["HeatsourceType"] in heatsource_using_exhaust_heat:
                        ...
                        resultJson["for_CGS"]["E_ref_cgsC_ABS_day"] += \
            ...
            if ref_name == resultJson["for_CGS"]["CGS_refName_H"]:
                ...
                # 当該温熱源群の主機の消費電力を差し引く。
                for unit_id, unit_configure in enumerate(inputdata["REF"][ref_name]["Heatsource"]):
                    resultJson["for_CGS"]["E_ref_main_MWh_day"] -= resultJson["REF"][ref_name]["Heatsource"][unit_id][
