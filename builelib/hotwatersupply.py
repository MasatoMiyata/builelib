...
def calc_energy(inputdata, DEBUG=False):
    ...
    Tout, _, Iod, Ios, Inn = climate.readHaspClimateData(climatedata_directory + "/" +
                                                         Area[inputdata["Building"]["Region"] + "地域"][
                                                             "気象データファイル名"])
    ...
    for unit_name in inputdata["HotwaterSupplySystems"]:
        ...
        for unit_id, unit_configure in enumerate(inputdata["HotwaterSupplySystems"][unit_name]["HeatSourceUnit"]):
            ...
            if DEBUG:
                ...
                print(
                    f'  - 給湯機器の効率 {inputdata["HotwaterSupplySystems"][unit_name]["HeatSourceUnit"][unit_id]["RatedEfficiency"]}')
    ...
    for room_name in inputdata["HotwaterRoom"]:
        ...
        # 日積算給湯量参照値 [L/day]
        inputdata["HotwaterRoom"][room_name]["hotwater_demand"] = hotwater_demand * inputdata["Rooms"][room_name][
            "roomArea"]
        inputdata["HotwaterRoom"][room_name]["hotwater_demand_washroom"] = hotwater_demand_washroom * \
                                                                           inputdata["Rooms"][room_name]["roomArea"]
        inputdata["HotwaterRoom"][room_name]["hotwater_demand_shower"] = hotwater_demand_shower * \
                                                                         inputdata["Rooms"][room_name]["roomArea"]
        inputdata["HotwaterRoom"][room_name]["hotwater_demand_kitchen"] = hotwater_demand_kitchen * \
                                                                          inputdata["Rooms"][room_name]["roomArea"]
        inputdata["HotwaterRoom"][room_name]["hotwater_demand_other"] = hotwater_demand_other * \
                                                                        inputdata["Rooms"][room_name]["roomArea"]

        # 各室の室使用スケジュール （＝室の同時使用率。 給湯需要がある室は、必ず空調されている前提とする）
        roomScheduleRoom, _, _, _, _ = \
            bc.get_roomUsageSchedule(inputdata["Rooms"][room_name]["buildingType"],
                                     inputdata["Rooms"][room_name]["roomType"], input_calendar)
        ...
        inputdata["HotwaterRoom"][room_name]["hotwater_demand_daily"] = \
            inputdata["HotwaterRoom"][room_name]["hotwater_demand"] * inputdata["HotwaterRoom"][room_name][
                "hotwaterSchedule"]

        inputdata["HotwaterRoom"][room_name]["hotwater_demand_washroom_daily"] = \
            inputdata["HotwaterRoom"][room_name]["hotwater_demand_washroom"] * inputdata["HotwaterRoom"][room_name][
                "hotwaterSchedule"]

        inputdata["HotwaterRoom"][room_name]["hotwater_demand_shower_daily"] = \
            inputdata["HotwaterRoom"][room_name]["hotwater_demand_shower"] * inputdata["HotwaterRoom"][room_name][
                "hotwaterSchedule"]

        inputdata["HotwaterRoom"][room_name]["hotwater_demand_kitchen_daily"] = \
            inputdata["HotwaterRoom"][room_name]["hotwater_demand_kitchen"] * inputdata["HotwaterRoom"][room_name][
                "hotwaterSchedule"]

        inputdata["HotwaterRoom"][room_name]["hotwater_demand_other_daily"] = \
            inputdata["HotwaterRoom"][room_name]["hotwater_demand_other"] * inputdata["HotwaterRoom"][room_name][
                "hotwaterSchedule"]
        ...
        if DEBUG:
            ...
            print(
                f'  - 日別給湯使用量（手洗い） {np.sum(inputdata["HotwaterRoom"][room_name]["hotwater_demand_washroom_daily"])}')
            print(
                f'  - 日別給湯使用量（シャワー） {np.sum(inputdata["HotwaterRoom"][room_name]["hotwater_demand_shower_daily"])}')
            print(
                f'  - 日別給湯使用量（厨房） {np.sum(inputdata["HotwaterRoom"][room_name]["hotwater_demand_kitchen_daily"])}')
            print(
                f'  - 日別給湯使用量（その他） {np.sum(inputdata["HotwaterRoom"][room_name]["hotwater_demand_other_daily"])}')
    ...
    for unit_name in inputdata["HotwaterSupplySystems"]:
        ...
        inputdata["HotwaterSupplySystems"][unit_name]["heatloss_coefficient"] = \
            thermal_conductivity_dict[inputdata["HotwaterSupplySystems"][unit_name]["PipeSizeType"]][
                inputdata["HotwaterSupplySystems"][unit_name]["InsulationType"]]
    ...
    for unit_name in inputdata["HotwaterSupplySystems"]:
                ...
                if unit_name == unit_configure["SystemName"]:
                    ...
                    if unit_configure["HotWaterSavingSystem"] == "無":
                        ...
                        inputdata["HotwaterSupplySystems"][unit_name]["Qs_eqp_daily"] += \
                            inputdata["HotwaterRoom"][room_name]["hotwater_demand_washroom_daily"] * 1.0 * \
                            unit_configure["roomPowerRatio"] + \
                            inputdata["HotwaterRoom"][room_name]["hotwater_demand_shower_daily"] * 1.0 * unit_configure[
                                "roomPowerRatio"] + \
                            inputdata["HotwaterRoom"][room_name]["hotwater_demand_kitchen_daily"] * 1.0 * \
                            unit_configure["roomPowerRatio"] + \
                            inputdata["HotwaterRoom"][room_name]["hotwater_demand_other_daily"] * 1.0 * unit_configure[
                                "roomPowerRatio"]

                    elif unit_configure["HotWaterSavingSystem"] == "自動給湯栓":

                        inputdata["HotwaterSupplySystems"][unit_name]["Qs_eqp_daily"] += \
                            inputdata["HotwaterRoom"][room_name]["hotwater_demand_washroom_daily"] * 0.6 * \
                            unit_configure["roomPowerRatio"] + \
                            inputdata["HotwaterRoom"][room_name]["hotwater_demand_shower_daily"] * 1.0 * unit_configure[
                                "roomPowerRatio"] + \
                            inputdata["HotwaterRoom"][room_name]["hotwater_demand_kitchen_daily"] * 1.0 * \
                        ...
                        inputdata["HotwaterSupplySystems"][unit_name]["Qs_eqp_daily"] += \
                            inputdata["HotwaterRoom"][room_name]["hotwater_demand_washroom_daily"] * 1.0 * \
                            unit_configure["roomPowerRatio"] + \
                            inputdata["HotwaterRoom"][room_name]["hotwater_demand_shower_daily"] * 0.75 * \
                            unit_configure["roomPowerRatio"] + \
                            inputdata["HotwaterRoom"][room_name]["hotwater_demand_kitchen_daily"] * 1.0 * \
                            unit_configure["roomPowerRatio"] + \
                            inputdata["HotwaterRoom"][room_name]["hotwater_demand_other_daily"] * 1.0 * unit_configure[
        ...
        if DEBUG:
            ...
            print(
                f'  - 日積算湯供給量（節湯込み） {np.sum(inputdata["HotwaterSupplySystems"][unit_name]["Qs_eqp_daily"])}')
    ...
    for unit_name in inputdata["HotwaterSupplySystems"]:
        ...
        if (inputdata["HotwaterSupplySystems"][unit_name]["SolarSystemArea"] != "") and (
                inputdata["HotwaterSupplySystems"][unit_name]["SolarSystemArea"] != None):
            # 日積算日射量 [Wh/m2/day]
        ...
        if DEBUG:
            print(f'機器名称 {unit_name}')
            print(
                f'  - 太陽熱利用システムの熱利用量 {np.sum(inputdata["HotwaterSupplySystems"][unit_name]["Qs_solargain"])}')
    ...
    for unit_name in inputdata["HotwaterSupplySystems"]:
        ...
        inputdata["HotwaterSupplySystems"][unit_name]["E_eqp"] = \
            inputdata["HotwaterSupplySystems"][unit_name]["Q_eqp"] / inputdata["HotwaterSupplySystems"][unit_name][
                "RatedEfficiency_total"]

        # 設計一次エネルギー消費量 [MJ/day]
        resultJson["設計一次エネルギー消費量[MJ/年]"] += np.sum(
            inputdata["HotwaterSupplySystems"][unit_name]["E_eqp"]) / 1000
    ...
    for unit_name in inputdata["HotwaterSupplySystems"]:
        ...
        resultJson["HotwaterSupplySystems"][unit_name]["湯使用量（節湯込）[L/年]"] = np.sum(
            inputdata["HotwaterSupplySystems"][unit_name]["Qs_eqp_daily"] / 1000)
        resultJson["HotwaterSupplySystems"][unit_name]["太陽熱利用量[MJ/年]"] = np.sum(
            inputdata["HotwaterSupplySystems"][unit_name]["Qs_solargain"] / 1000)
        resultJson["HotwaterSupplySystems"][unit_name]["給湯加熱負荷[MJ/年]"] = np.sum(
            inputdata["HotwaterSupplySystems"][unit_name]["Q_eqp"] / 1000)
        resultJson["HotwaterSupplySystems"][unit_name]["配管熱損失[MJ/年]"] = np.sum(
            inputdata["HotwaterSupplySystems"][unit_name]["Qp_eqp"] / 1000)
        resultJson["HotwaterSupplySystems"][unit_name]["設計一次エネルギー消費量[MJ/年]"] = np.sum(
            inputdata["HotwaterSupplySystems"][unit_name]["E_eqp"] / 1000)
    ...
    for unit_name in inputdata["HotwaterSupplySystems"]:
        resultJson["HotwaterSupplySystems"][unit_name]["設計値/基準値"] = \
            resultJson["HotwaterSupplySystems"][unit_name]["設計一次エネルギー消費量[MJ/年]"] / \
            resultJson["HotwaterSupplySystems"][unit_name]["基準一次エネルギー消費量[MJ/年]"]
        ...
        for cgs_name in inputdata["CogenerationSystems"]:
            ...
            for unit_name in inputdata["HotwaterSupplySystems"]:
                ...
                if inputdata["CogenerationSystems"][cgs_name]["HowWaterSystem"] != "" and \
                    ...
                    # コジェネの排熱利用先であれば
                    for dd in range(0, 365):
                        Edesign_MJ_CGS_hour[dd] += inputdata["HotwaterSupplySystems"][unit_name]["E_eqp"][
                                                       dd] / 24 / 1000 * np.ones(24)
                        Q_eqp_CGS_hour[dd] += inputdata["HotwaterSupplySystems"][unit_name]["Q_eqp"][
                                                  dd] / 24 / 1000 * np.ones(24)
                    ...
                    for unit_id, unit_configure in enumerate(
                            inputdata["HotwaterSupplySystems"][unit_name]["HeatSourceUnit"]):

                        if unit_configure["UsageType"] == "給湯負荷用":
                            if unit_configure["HeatSourceType"] == "電気瞬間湯沸器" or unit_configure[
                                "HeatSourceType"] == "貯湯式電気温水器" or \
                                    unit_configure["HeatSourceType"] == "業務用ヒートポンプ給湯機" or unit_configure[
                                "HeatSourceType"] == "家庭用ヒートポンプ給湯機":

                                for dd in range(0, 365):
                                    Edesign_MWh_Ele_hour[dd] += inputdata["HotwaterSupplySystems"][unit_name]["E_eqp"][
