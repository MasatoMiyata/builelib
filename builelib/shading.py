...
def calc_shading_coefficient(area, direction, x1, x2, x3, y1, y2, y3, zxp, zxm, zyp, zym):
            ...
            if sin_hsdt_ALL[dd][hh] > 0 and iod_all_10min[dd][hh] > 0:
                ...
                if (Azwj_ALL[dd][hh] >= -90 and Azwj_ALL[dd][hh] <= 0):  # x+側
                    Awind_j_xp[dd][hh] = func_14(x2, x3, y1, y2, zxp, zyp, tan_hsdt_ALL[dd][hh], cos_Azwj_ALL[dd][hh],
                                                 tan_Azwj_ab_ALL[dd][hh])

                elif (Azwj_ALL[dd][hh] > 0 and Azwj_ALL[dd][hh] <= 90):  # x-側
                    Awind_j_xm[dd][hh] = func_18(x2, x1, y1, y2, zxm, zyp, tan_hsdt_ALL[dd][hh], cos_Azwj_ALL[dd][hh],
                                                 tan_Azwj_ALL[dd][hh])
    ...
    ## 日除け効果係数（冷房期、暖房期）
    r_wind_SUM = (
                             r_dsr_j_xp_SUM * ST_xp_SUM + r_dsr_j_xm_SUM * ST_xm_SUM + r_isr_j_yp * ST_st_SUM + r_isr_j_ym * ST_rt_SUM) \
                 / (ST_xp_SUM + ST_xm_SUM + ST_st_SUM + ST_rt_SUM)
    r_wind_WIN = (
                             r_dsr_j_xp_WIN * ST_xp_WIN + r_dsr_j_xm_WIN * ST_xm_WIN + r_isr_j_yp * ST_st_WIN + r_isr_j_ym * ST_rt_WIN) \
