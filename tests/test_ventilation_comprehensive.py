...
# テストケースファイルのディレクトリ
TEST_DATA_DIR = Path(__file__).parent / "ventilation_comprehensive"

# 正解値ファイル（列3: 設計一次エネ[MJ/年], 列4: 基準一次エネ[MJ/年]）
...
def get_test_data():
    ...
    for row in rows[1:]:
        case_id = row[0]  # 'Case00' など
        num = case_id[4:]  # '00' など

        xlsm_path = TEST_DATA_DIR / f"Case_vt_comprehensive_{num}.xlsm"
        if not xlsm_path.exists():
            continue

        expected_design = convert2number(row[3], 0)    # 設計一次エネルギー消費量[MJ/年]
        expected_standard = convert2number(row[4], 0)  # 基準一次エネルギー消費量[MJ/年]
