...
# テストケースファイルのディレクトリ
TEST_DATA_DIR = Path(__file__).parent / "airconditioning_window"

# 正解値ファイル（列12: 設計値[MJ/m2年], 列13: 基準値[MJ/m2年]）
...
def get_test_data():
    ...
    for row in rows[1:]:
        ...
        xlsm_path = TEST_DATA_DIR / f"Case_ac_window_{num}.xlsm"
        if not xlsm_path.exists():
            continue

        expected_design = convert2number(row[12], 0)    # 設計一次エネルギー消費量[MJ/m2年]
        expected_standard = convert2number(row[13], 0)  # 基準一次エネルギー消費量[MJ/m2年]
