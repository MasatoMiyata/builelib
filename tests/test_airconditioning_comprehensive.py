...
# テストケースファイルのディレクトリ
TEST_DATA_DIR = Path(__file__).parent / "airconditioning_comprehensive"

# 正解値ファイル（F列: 設計値[MJ/m2年], G列: 基準値[MJ/m2年]）
...
def get_test_data():
    ...
    for row in rows[1:]:
        ...
        xlsm_path = TEST_DATA_DIR / f"Case_ac_comprehensive_{num}.xlsm"
        if not xlsm_path.exists():
            continue

        expected_design = convert2number(row[5], 0)    # F列: 設計一次エネルギー消費量[MJ/m2年]
        expected_standard = convert2number(row[6], 0)  # G列: 基準一次エネルギー消費量[MJ/m2年]
...
def test_calc(xlsm_path, expected_design, expected_standard):
    ...
    # 設計値: 最大差 ~0.44%（Case07）のため rel=0.005 を使用
    assert actual_design == pytest.approx(expected_design, rel=0.005, abs=0.001)
    # 基準値: 差はほぼ 0% のため rel=0.001 を使用
    assert actual_standard == pytest.approx(expected_standard, rel=0.001, abs=0.001)
